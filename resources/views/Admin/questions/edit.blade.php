@extends('layouts.app', ['activePage' => 'Question-management', 'titlePage' => __('Question Management')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('questions.update', $question) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('PATCH')
            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Update Question') }}</h4>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" id="input-title" type="text" placeholder="{{ __('title') }}" value="{{ $question->title }}" required="true" aria-required="true"/>
                      @if ($errors->has('title'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <div class="form-group{{ $errors->has('answers') ? ' has-danger' : '' }}">
                      <textarea class="form-control{{ $errors->has('answers') ? ' is-invalid' : '' }}" name="answers" id="input-answers" type="text" value="" required="true" aria-required="true">
                        {{ $question->answers }}
                      </textarea>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <div class="form-group{{ $errors->has('right_answer') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('right_answer') ? ' is-invalid' : '' }}" name="right_answer" id="input-right-answer" type="text" placeholder="{{ __('right_answer') }}" value="{{ $question->right_answer }}" required="true" aria-required="true"/>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('score') ? ' has-danger' : '' }}">
                      <select class="form-control" name="score" value='{{ $question->score }}' required>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('quiz_id') ? ' has-danger' : '' }}">
                      ...
                      @foreach(\App\Quiz::orderBy('id', 'desc')->get() as $quiz)
                          ...
                          <option 
                          <?php if($question->quiz->id == $quiz->id)
                              echo 'selected';
                          
                          ?>
                          value="{{ $quiz->id }}">{{ Str::limit($quiz->name, 25) }}</option>
                          @endforeach
                      </select>
